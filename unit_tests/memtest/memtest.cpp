...
__host__ void main() {
   ...
   for (uint i=0; i<n_objs; ++i) {
      storage.push_back(testStructure(100));
   }
   ...
   for (uint j=0; j<n_loops; ++j) {
      size_t newSize = initial_size * std::power(2,j);
 ...
 */
   int gpu_reportMemory(const std::vector<testStructure> objs) {
   size_t o_size=0;
   size_t o_cap=0;
   for (uint i=0; i<objs.size(); ++i) {
      o_size += objs[i].size();
      o_cap += objs[i].capacity();
   }
